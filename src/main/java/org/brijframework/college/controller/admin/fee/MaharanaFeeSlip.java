...
public class MaharanaFeeSlip extends AbstractPdfView {
	@Override
	...
	protected void buildPdfDocument(Map<String, Object> model,
		...
		for (StudentDTO studentDTO : commonDTO.getStudentDTOs()) {
			...
			mother=studentDTO.getMotherName();
		}
		...
		pCell = new PdfPCell();
		pCell.setBorder(Rectangle.NO_BORDER);
		pCell.addElement(new Paragraph(String.valueOf(commonDTO.getReceiptNo()), font));
		table3.addCell(pCell);
		...
		pCell = new PdfPCell();
		pCell.setBorder(Rectangle.NO_BORDER);
		pCell.addElement(new Paragraph("Mother's Name", font));
		table3.addCell(pCell);
		pCell = new PdfPCell();
		pCell.setBorder(Rectangle.NO_BORDER);
